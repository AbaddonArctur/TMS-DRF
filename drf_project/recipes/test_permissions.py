...
class PermissionsTestCase(TestCase):
    def setUp(self):
        ...
        token = self.client.post(
            reverse("token_obtain_pair"), {"username": "author", "password": "1"}
        ).data["access"]
        self.author_headers = {"HTTP_AUTHORIZATION": f"Bearer {token}"}

        token2 = self.client.post(
            reverse("token_obtain_pair"), {"username": "other", "password": "1"}
        ).data["access"]
    ...
    def test_author_can_update(self):
        ...
        resp = self.client.patch(
            url, {"title": "New title"}, format="json", **self.author_headers
        )
    ...
    def test_other_cannot_update(self):
        ...
        resp = self.client.patch(
            url, {"title": "Wrong edit"}, format="json", **self.other_headers
        )
